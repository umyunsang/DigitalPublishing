...
```
wheel/touch/pointer
  → Lenis (스무스 스크롤, GSAP ticker 동기)
  → ScrollTrigger (핀/스크럽/진행)
  → 활성 section.enter()/scrub  +  webgl.js가 활성 씬만 scissor/viewport로 draw
  → 섹션 경계: Observer/snap 감지 → transitions.js 타임라인 재생
```
...
### 4.4 `engine/transitions.js`
- **역할**: 섹션 경계 "애니메이티브 전환"(clipPath/SVG mask/카메라 무브), 허브 radial 감성 계승. GSAP Observer로 풀스냅(D). 긴 내부 스크롤 섹션(why-wdc)은 내부 스크롤 후 경계에서만 스냅하는 하이브리드 예외.
- **인터페이스**: `enableSnap(sections, opts)`, `playBoundary(fromIdx, toIdx)`.
- **의존**: gsap, Observer.

### 4.5 `sections/<name>.js` (7개)
- 각 Codrops 데모의 비주얼을 통일 엔진 위로 재작성. WebGL 섹션은 자체 렌더러를 만들지 않고 `engine/webgl.js`에 씬을 등록.
...
- 키보드 네비게이션(↑/↓/PageUp/Down, 섹션 점프), 포커스 관리.
- 모바일 터치 임계값 튜닝(Observer `tolerance/dragMinimum`).
- `why-wdc`(긴 내부 스크롤)는 풀스냅 예외 처리.

---

## 8. 테스트 (Playwright 기설정 활용)
- 섹션별 스크롤 진입/이탈 동작 및 시각 스냅샷.
- `dispose` 후 WebGL 컨텍스트/메모리 누수 검증(반복 진입·이탈).
- Lighthouse 성능 예산 검사.
- `prefers-reduced-motion` 경로 스모크.
- 크로스브라우저(Chromium/Firefox/WebKit) 폴백 스모크.
- 허브 메뉴 앵커 → 해당 섹션 진입 검증.

---

## 9. 단계적 빌드 (전체 7섹션 목표, 안전한 순서)
- **P0 — 엔진 골격**: `journey/index.html` 셸, `engine/*`, CSS 스코프·토큰, 빈 7섹션 + 스크롤/스냅 동작.
- **P1 — 비-WebGL 섹션**: 01-arrival, 02-saved-scenes, 03-why-wdc, 06-design-city 이식.
- **P2 — WebGL 통합**: 04·05·07을 공유 렌더러로 통합, 씬 register/dispose.
- **P3 — 전환 연출**: 경계 transitions·Observer 스냅·radial 감성 폴리시.
- **P4 — 마감**: 07 자산 다이어트, 성능 예산 달성, 접근성·폴백·테스트 완비, 허브 메뉴 링크 교체.
