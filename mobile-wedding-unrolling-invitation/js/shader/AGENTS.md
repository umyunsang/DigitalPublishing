...
| Uniform progress | `../app.js` | `Item.render()` writes `mesh.material.uniforms.progress.value` from current scroll/view position. |

## CONVENTIONS

- Shader files are imported directly from JavaScript; Parcel must process `.glsl`.
- `progress` is the main animation scalar, normally `0` to `1`, and should remain deterministic for a given scroll/view position.
...
- Do not change uniform names without updating `rolls.js` material setup and `app.js` uniform writes.
