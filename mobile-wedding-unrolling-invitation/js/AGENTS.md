...
The `js/` folder is the runtime behavior layer: preload, smooth scrolling, IntersectionObserver visibility gating, scroll-derived shader progress, Three.js scene creation, and vendored image loading.
...
| Animated image state | `app.js` | `Item` measures DOM images, creates meshes, and derives `progress` from the current scroll/view position. |
...
- DOM contracts are implicit: `#container`, `main`, `div[data-scroll]`, and `.js-image`.
- Runtime state is mutable and instance-local; this is demo code, not a typed data model.
- `Item.render()` writes `mesh.material.uniforms.progress.value` directly from scroll-derived progress; Three.js render reads current uniforms.
...
- `app.js`: `IntersectionObserver` is a hard dependency for mesh visibility and render scheduling, but must not own roll/unroll state.
